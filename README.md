...
(the connection switch) + a matching flow-behavior table (`D_Water.json`, `D_Energy.json`, etc.).
...
**`D_Resource.json` uses this exact same `bHasXConnection` + `XFlow` pattern for six resource
types, not just water** *(VERIFIED against the struct's `Defaults`)* — everything documented in
this section for water applies identically to the others, just swap the table name:

| Resource | Connection flag | Flow pointer | Flow-behavior table |
|---|---|---|---|
| Water | `bHasWaterConnection` | `WaterFlow` | `Traits/D_Water.json` |
| Energy | `bHasEnergyConnection` | `EnergyFlow` | `Traits/D_Energy.json` |
| Fuel | `bHasFuelConnection` | `FuelFlow` | `Traits/D_Fuel.json` |
| Oxygen | `bHasOxygenConnection` | `OxygenFlow` | `Traits/D_Oxygen.json` |
| Crude Oil | `bHasCrudeOilConnection` | `CrudeOilFlow` | `Traits/D_CrudeOil.json` |
| Refined Oil | `bHasRefinedOilConnection` | `RefinedOilFlow` | `Traits/D_RefinedOil.json` |

The other five flow-behavior tables haven't been individually confirmed to use the same
`Store`/`Produce`/`Consume` enum values as `D_Water.json` — **INFERRED**, not yet verified, but a
reasonable starting assumption given the identical parent-struct pattern.
...
### Untested lead: `bAllowPullingResourceFromInventory` / `bAllowPushingResourceToInventory` *(UNTESTED)*

`D_Water.json`'s full `Defaults` block includes two boolean fields not covered above:
`bAllowPullingResourceFromInventory` and `bAllowPushingResourceToInventory` (both default
`false`, unset on every vanilla row checked so far). These look like plausible candidates for
tying a node's network flow to its own `Fillable`/`Generator`-accumulated stock — i.e. a
genuine "only gives out what it's actually collected" behavior, which nothing in the confirmed
`Store`/`Produce`/`Consume` model above achieves (`Produce` is unconditional; `Store` doesn't
push at all). **Not yet tested in-game** — worth an isolated experiment before relying on it,
but flagged here so it isn't lost as a next step.
